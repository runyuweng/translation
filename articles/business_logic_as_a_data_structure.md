...
```
通过手动使用命令式的JavaScript来管理你所有的UI在现在是不常见、相对落后的，这种情况的出现是有着充分的理由的。使用这种方式开发对开发者来说绝对是一种痛苦的体验。幸运的是在前端领域有很多的创新。从模板到由虚拟dom实现的JSX语法，让我们能够以一种声明的方式去表示动态UI。
> “声明式的代码描述了程序是什么，然而命令式的代码描述了JavaScript应该怎么做”

在本文中，我们将思考用哪些常见的数据结构可以编写声明式的代码，然后思考是否使用这种方式是否有利于我们业务逻辑的编写。
